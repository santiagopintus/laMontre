...
export default function MobileMenu({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      >
        <span
          className={`block h-0.75 w-7 origin-center bg-[#212529] transition-transform duration-300 ease-[cubic-bezier(0.62,-0.99,0.54,1.9)] ${
            isOpen ? 'translate-y-2.25 rotate-45' : ''
          }`}
        />
        <span
          className={`block h-0.75 w-7 bg-[#212529] transition-transform duration-300 ease-[cubic-bezier(0.62,-0.99,0.54,1.9)] ${
            isOpen ? 'scale-0' : ''
          }`}
        />
        <span
